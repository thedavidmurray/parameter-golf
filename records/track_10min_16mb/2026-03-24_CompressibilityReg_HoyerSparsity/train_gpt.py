...
Core insight: parameter-golf submissions are compressed with zlib after int8 quantization.
Per-row int8 quantization normalizes each row by its own maximum, so the absolute weight
scale doesn't matter — only the *shape* of the distribution within each row.

  Gaussian rows  → max entropy after quantization → barely compressible (~1.0x ratio)
  Sparse rows    → low entropy                    → much better compression (1.5-2x+)

This script adds:
  1. compression_regularizer()   Penalizes the Hoyer L1/L2 ratio per weight row.
                                 Minimizing L1/L2 pushes rows toward spiky distributions
                                 (few dominant values), reducing int8 byte-stream entropy.

  2. measure_compression_metrics()   Samples weight matrices, measures actual Hoyer ratio
                                     and estimated zlib compression ratio during training.

  3. budget_analysis()   Given the current compression ratio, computes max parameters
                         that fit in the 16MB budget and the optimal architecture.

Why Muon needs an antidote:
  Muon's Newton-Schulz step drives weight matrices toward near-orthogonal structure —
  which is the LEAST compressible (uniform singular values = maximum entropy rows).
  The Hoyer regularizer counteracts this for compressibility without changing the optimizer.

Key hyperparameters:
  COMP_REG_WEIGHT         (default 1e-3) regularizer coefficient. Try 5e-4, 1e-3, 2e-3.
  COMP_REG_WARMDOWN_ONLY  (default 1)    only apply during warmdown (safe starting point).
  COMPRESSION_LOG_EVERY   (default 1000) steps between compression metric logs.
"""
...
@torch.no_grad()
def measure_compression_metrics(model: nn.Module) -> dict:
    """
    Computes Hoyer ratio for all large weight matrices and estimates compression ratio.
    Safe to call during training (no_grad). Returns dict for logging.
    """
    hoyer_vals: list[float] = []
    raw_bytes_total = 0
    est_compressed_total = 0
    ...
    for name, p in model.named_parameters():
        ...
        l2 = t.norm(dim=1).clamp_min(1e-8)
        l1 = t.abs().sum(dim=1)
        hoyer = float((l1 / (l2 * t.shape[1] ** 0.5)).mean().item())
        hoyer_vals.append(hoyer)

        # Simulate int8 quantization and estimate compressed size via entropy
        clip_abs = torch.quantile(t.abs(), INT8_CLIP_Q, dim=1)
        clipped = torch.clamp(t, -clip_abs[:, None], clip_abs[:, None])
        scale = (clip_abs / 127.0).clamp_min(1.0 / 127.0)
        q = torch.clamp(torch.round(clipped / scale[:, None]), -127, 127).to(torch.int8)
        raw = q.numpy().tobytes()
        # Shannon entropy → estimated bits/byte
        counts = {}
        for b in raw:
            counts[b] = counts.get(b, 0) + 1
        n = len(raw)
        h = -sum((c / n) * math.log2(c / n) for c in counts.values()) if counts else 8.0
        raw_bytes_total += n
        est_compressed_total += int(n * h / 8.0)

    if not hoyer_vals:
        return {"hoyer_mean": float("nan"), "est_compress_ratio": 1.0}

    hoyer_mean = sum(hoyer_vals) / len(hoyer_vals)
    est_ratio = raw_bytes_total / max(est_compressed_total, 1)
    return {"hoyer_mean": hoyer_mean, "est_compress_ratio": est_ratio, "n_matrices": len(hoyer_vals)}


def budget_analysis(est_compress_ratio: float, bits: int = 8,
                    total_budget: int = 16_000_000, code_bytes: int = 60_000,
                    vocab: int = 1024, dim: int = 512,
                    num_heads: int = 8, num_kv_heads: int = 4, mlp_mult: float = 2.0) -> str:
    """
    (C methodology) Given the measured compression ratio, compute how many parameters fit
    in the budget and what the optimal architecture looks like.

    This is the architecture-search-from-compression-space framing: instead of choosing
    (layers, dim) and hoping it fits, we start from the byte budget and work backwards.
    """
    model_budget = total_budget - code_bytes
    # Bytes per param after quantization+compression
    bytes_per_param = (bits / 8.0) / est_compress_ratio
    # Per-row scale factors: ~layers * matrices_per_layer * dim * 2 bytes (fp16)
    # Estimated for this architecture
    scale_oh_per_layer = 6 * dim * 2  # 6 matrices per block, fp16 scales
    ...
    mlp_hidden = int(mlp_mult * dim)
    mlp_params = dim * mlp_hidden + mlp_hidden * dim
    per_layer_bytes = (attn_params + mlp_params) * bytes_per_param + scale_oh_per_layer
    max_layers = int((model_budget - embed_bytes) / per_layer_bytes)
    max_params = embed_params + max_layers * (attn_params + mlp_params)

    return (
        f"budget_analysis: compress={est_compress_ratio:.3f}x int{bits}  "
        f"max_layers={max_layers} max_params={max_params/1e6:.1f}M  "
        f"({bytes_per_param*1000:.2f} bytes/Kparam)"
    )
...
class MLP(nn.Module):
    # LeakyReLU(0.5)² confirmed -0.003 BPB vs ReLU² in SOTA records.
...
def main() -> None:
    ...
    while True:
        ...
        # Compression metrics log (independent of validation cadence, cheaper).
        should_log_compression = (
            master_process
            and args.compression_log_every > 0
            and step > 0
            and step % args.compression_log_every == 0
        )
        if should_log_compression:
            cm = measure_compression_metrics(base_model)
            log0(
                f"step:{step} hoyer_mean:{cm['hoyer_mean']:.4f} "
                f"est_compress_ratio:{cm['est_compress_ratio']:.3f}x  "
                + budget_analysis(cm["est_compress_ratio"], bits=8,
    ...
    # -----------------------------
    # Measure the achieved compression metrics after training and report the optimal
    # architecture for the NEXT run at this compression level. This is the C methodology:
    # architecture search structured around compressed bytes, not raw parameter counts.
